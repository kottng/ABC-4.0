...
В нашем случае мы имеем 1 производителя - метод customerCame (описывает, как посетитель меняет очередь), и 1 потребитель - метод workerWork (описывает, как парикмахер работает с очередью). Программа базируется на канале - очереди(in_line). Количество посетителей варьируется от нуля и задается при помощи одного из форматов ввода: через консоль, файл или при помощи сгенерированного случайно числа. Программа завершается, когда кончатся посетители.
